...
# ------------- wroking with the best params -----------------
    ...
    def predict_digit(image):
        ...
        if isinstance(shap_values, list):
            shap_for_class = shap_values[predicted_class][0][0]
        else:
            shap_for_class = shap_values[0, 0, :, :, predicted_class]

        if torch.is_tensor(shap_for_class):
            shap_for_class = shap_for_class.detach().cpu().numpy()

        img_np = np.array(original_image)

        plt.figure(figsize=(5, 5))
        plt.imshow(img_np, cmap="gray")
        plt.imshow(shap_for_class, cmap="jet", alpha=0.55)
        plt.axis("off")
        plt.title(f"Obszary wpływające na decyzję modelu: {predicted_class}")
        plt.savefig("shap_explanation.png", bbox_inches="tight", pad_inches=0.1)
        plt.close()

        return {str(i): float(probs[i]) for i in range(10)}, "shap_explanation.png"
    ...
    demo = gr.Interface(
        ...
        outputs=[
            gr.Label(num_top_classes=10, label="Wynik klasyfikacji"),
            gr.Image(type="filepath", label="Mapa wpływu pikseli SHAP")
        ],
        title="Klasyfikacja cyfr MNIST z użyciem CNN",
        description="Narysuj cyfrę od 0 do 9. Model rozpozna cyfrę, a mapa SHAP pokaże, które obszary obrazu miały największy wpływ na decyzję."
    )
